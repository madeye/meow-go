...
void main() {
  group('SettingsScreen runtime section', () {
    testWidgets('shows Allow LAN switch with value from getConfigs', (
      tester,
    ) async {
      ...
      final sw = tester.widget<Switch>(
        find.descendant(
          of: find.byKey(const Key('switch_allow_lan')),
    ...
    testWidgets('shows IPv6 switch with value from getConfigs', (tester) async {
      ...
      final sw = tester.widget<Switch>(
        find.descendant(
          of: find.byKey(const Key('switch_ipv6')),
          matching: find.byType(Switch),
        ),
      );
      expect(sw.value, isTrue);
    ...
    testWidgets('toggling Allow LAN calls patchConfigs with allow-lan key', (
      tester,
    ) async {
    ...
    testWidgets('toggling IPv6 calls patchConfigs with ipv6 key', (
      tester,
    ) async {
      Map<String, dynamic>? patched;
      await tester.pumpWidget(
